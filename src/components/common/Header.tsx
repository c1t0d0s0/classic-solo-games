...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="w-full bg-slate-900/95 backdrop-blur border-b border-slate-800 sticky top-0 z-40 shadow-md">
      ...
      <div className="sm:hidden px-2 pb-2 pt-0.5 w-full">
        <nav className="grid grid-cols-4 gap-1 bg-slate-950/70 p-1 rounded-xl border border-slate-800/80 shadow-inner">
          {navItems.map((item) => (
            <button
              key={item.id}
              onClick={() => {
              ...
              className={`flex items-center justify-center gap-1 px-1 py-1.5 rounded-lg text-xs font-bold whitespace-nowrap transition-all active:scale-95 ${
                activeGame === item.id
